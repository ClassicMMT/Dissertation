...
np.random.seed(124)
sample_sizes = np.array([50, 100, 150, 200, 250, 300, 400, 500])
sample_sizes = np.array([50])


x1 = np.random.normal(loc=0, scale=5, size=(sample_size, 1))
x2 = np.vstack(
    (
...
for points_to_remove in points_removed:
    y_sub = y[points_to_remove:-points_to_remove]
    x_sub = x[points_to_remove:-points_to_remove, :]
...
full_data.to_csv("data/sample_size_experiment.csv", index=False)
...
if True:
    ...
    plt.title("Sample Size Experiment")
...
if True:
    ...
    plt.xlabel("Points Removed")
    plt.ylabel("Wasserstein")
    plt.title("Sample Size Experiment - By Class (Class 0 is removed)")
    plt.show()
