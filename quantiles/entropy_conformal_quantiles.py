...
# instantiate model
model = GenericNet(layers=[2, 1024, 512, 256, 2], activation="relu", random_state=random_state).train().to(device)
...
for lam in candidate_lambdas:
    mask = calib_confidences >= lam
    n_keep = mask.sum().item()
    if n_keep == 0:
        continue

    empirical_risk = (calib_errors[mask].sum() / n_keep).item()

    # get conservative upper bound on empirical risk
    r = calib_errors[mask].sum().item()
...
if True:
    ...
    fig.suptitle(f"Points coloured by predicted probability >= q_hat")
    plt.tight_layout()
    plt.show()
