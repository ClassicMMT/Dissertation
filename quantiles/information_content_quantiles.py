"""
This is the same experiment as entropy_quantiles.py except that it uses information content instead.
"""
...
for epoch in range(n_epochs):
    ...
    for features, labels in train_loader:
        ...
        # calculate information content for the final epoch
        if epoch + 1 == n_epochs:
            info_content = calculate_information_content(logits)
            train_info_content.append(info_content)

train_info_content = torch.cat(train_info_content)
train_q_hat = torch.quantile(train_info_content, 1 - 0.05, interpolation="higher")
...
# get information content for the calibration set
with torch.no_grad():
    model.eval()
    calib_info_content = []
    for features, labels in calib_loader:
        ...
        logits = model(features)
        info_content = calculate_information_content(logits)
        calib_info_content.append(info_content)

    calib_info_content = torch.cat(calib_info_content)


# find q_hat
alpha = 0.05
q_hat = torch.quantile(calib_info_content, 1 - alpha, interpolation="higher")


# experiment on x_test
with torch.no_grad():
    """
    certain_predictions here means predictions where the info_content is < q_hat
    uncertain_predictions means predictions where info_content >= q_hat
    """
    model.eval()
    ...
    for features, labels in test_loader:
        ...
        info_content = calculate_information_content(logits)
        info_content_over_q_hat = info_content >= q_hat
        ...
        # accuracies
        uncertain_preds = is_correct_pred[info_content_over_q_hat]
        uncertain_accuracy.append(uncertain_preds)
        certain_preds = is_correct_pred[~info_content_over_q_hat]
        certain_accuracy.append(certain_preds)

        # points
        uncertain_points.append(features[info_content_over_q_hat].cpu())
        uncertain_labels.append(labels[info_content_over_q_hat].cpu())
        certain_points.append(features[~info_content_over_q_hat].cpu())
        certain_labels.append(labels[~info_content_over_q_hat].cpu())

        all_info_content.append(info_content)
    ...
    all_info_content = torch.cat(all_info_content)


# accuracy of points with info_content < q_hat
...
(all_info_content > q_hat).float().mean()

# plot the whole information content distribution
if True:
    plt.axvline(q_hat, color="red")
    plt.hist(all_info_content, bins=50)
...
for name, attack in attacks.items():
    ...
    with torch.no_grad():
        ...
        adversarial_info_content = calculate_information_content(logits)
        results[name] = adversarial_info_content.cpu()
        labels[name] = (adversarial_info_content >= q_hat).cpu()


for name, adversarial_info_content in results.items():
    percent_over_q_hat = (adversarial_info_content > q_hat).float().mean()
...
if True:
    ...
    fig.suptitle(f"Points coloured by info_content > q_hat")
    plt.tight_layout()
    plt.show()
