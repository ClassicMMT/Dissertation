...
% CUSTOM
\setlength{\oddsidemargin}{1.36cm}
\setlength{\evensidemargin}{0cm}
\setlength{\topmargin}{1mm}
\setlength{\headheight}{1.36cm}
\setlength{\headsep}{1.00cm}
...
Originating in cheminformatics, the concept of an applicability domain (AD) defines a framework for determining regions in which a model's predictions can be considered reliable. Although approaches to defining the process of finding the AD vary \cite{LuqueRuiz2018}, the prominent work in \cite{Hanser2016ApplicabilityDomain} describes a simple three-step criterion to determine which points are outside of the AD based on their applicability, reliability and decidability. The purpose of this is to reject predictions for chemicals or molecules that fall outside the AD, since a model's predictions on these instances are almost certainly unreliable. However, applications of the AD concept in general machine learning remain limited although there have been many related works examining out-of-distribution (OOD) detection \cite{Hendrycks2017OOD}, novelty detection \cite{Marsland2003NoveltyDetection}, and outlier detection \cite{Tahir01022025}. There is a subtle, yet important, difference within each of these problems broken down into two types.

The work in \cite{Mathea2016} distinguishes between uncertainty through the features space (novelty detection) and the expected reliability of predictions (confidence estimation). Note that both of these are related but distinct problems. \textit{Novelty detection} considers a point as novel when it is outside the domain of the data. This point is not necessarily wrong or incorrect, the model has just not seen similar points before (for related methods see \cite{Mathea2016}). In contrast, \textit{confidence estimation} aims to use the class labels to estimate prediction confidence \cite{Vanderlooy2007}, since points located in regions with class-overlap should have higher uncertainty \cite{Mathea2016}. There are many methods for estimating confidence including using statistical methods such as posterior probabilities or naive Bayesian classifier \cite{Duda2001}, machine learning techniques such as k-nearest neighbours, decision trees, random forests, support vector machines, or neural networks \cite{Mathea2016}. However, often-times model's confidence does not correspond with the true likelihood of correctness so confidence calibration techniques have been developed to align the predicted probabilities with the true likelihood of correctness \cite{confidence_calibration, silva2023classifier}. After obtaining prediction confidences, various methods can be used for abstaining from predictions. 

The works in \cite{rejection_survey, geifman2017selective} review existing literature on techniques to enable models with a reject option when a model's confidence is lacking or it falls outside of the AD. The work in \cite{mao2024predictorrejectormulticlassabstentiontheoretical} categorises abstention into confidence-based methods \cite{ni2019calibrationmulticlassclassificationrejection}, selective classification with a predictor and selector \cite{el-yaniv2010Onthefoundationsofnoise-freeselectiveclassification}, or predictor and rejector \cite{mohri2023learningrejectfixedpredictor}, and score-based methods \cite{mao2024theoreticallygroundedlossfunctions}. However, while many techniques have been developed, the problem with all these approaches is that many require class labels which may not be available and therefore determining regions of uncertainty based on the feature space or predicted class probabilities becomes important. 

The work in \cite{Ma2018CharacterizingAdversarialSubspaces} developed a detector used to define adversarial subspaces, finding that these subspaces exist across all layers in a network, and that a specially trained classifier was able to distinguish between normal and adversarial points on five adversarial attacks. This is relevant as adversarial points generated by an attack are artificial versions of ``natural'' (test-time) adversarial points which exist in regions of class-overlap as discussed above. However, the authors only examined the use of this detector on generated adversarial examples and did not consider whether the detector could identify points of uncertainty. Likewise, the detector utilises distances measures which have been found to be outperformed by confidence-based measures \cite{Mathea2016}. The recent work in \cite{zhao2025revealingvulnerableregions} developed an adversarial attack using differential evolution (\cite{storn1997differential}) to identify pixel-level regions of vulnerability for image models, showing that the attack is able to identify vulnerable regions in both the background and foreground. This implies that these regions are areas of uncertainty, since one image may be one class, but if one pixel is changed slightly, the decision boundary is crossed and misclassification occurs. However, while this work is promising for image data, they did not design their algorithm to capture inherent uncertainty found within the class distribution.

Generally, uncertainty can be roughly broken down into two parts. \textit{Epistemic} uncertainty comes from having incomplete information about which model or architecture is the best fit for the data we have on hand, and \textit{aleatoric} uncertainty is uncertainty coming from variation that we are unable to account for, such as image bluriness or rotation \cite{bhatt2021uncertaintyformtransparencymeasuring}. Epistemic uncertainty and aleatoric uncertainty are also known as reducible or irreducible uncertainty, respectively. For the former, one can reduce uncertainty with more data since model architecture can be better-informed. For the latter, increasing the number of available examples does does not make the existing blurry points already blurry. However, assuming a trained classifier has learnt a good approximation of the true decision boundary, points which are ``uncertain'' are generally located near the boundaries where the data are sparse, or outside the domain of the data which directly corresponds to the AD \cite{Mathea2016}. 
The work in \cite{logit_dist_adv_trained_dnn} examined the logit-distributions of adversarially-trained networks, finding that adversarial training \cite{goodfellow2015} (training on both normal and adversarial examples) lowers the maximum logit value and shrinks the logit gaps. This means the distribution of the gaps becomes less spread, with lower mean, a stronger positive skew, leading to less confident predictions and more effort by the model to learn the true underlying features \cite{logit_dist_adv_trained_dnn}. The authors proposed this occurs due to the regularisation that implicitly occurs during adversarial training, with models learning to avoid being extremely confident by shrinking the high-confidence margins. However, the authors found that while adversarial training improves performance on adversarial examples, normal examples are adversely impacted on the CIFAR-10 AND CIFAR-100 datasets. 
The work in \cite{shen2022posthocuncertaintylearningusing} proposed that machine learning models are often overconfident making uncertainty estimates unreliable. The authors proposed a post-hoc method which does not require model retraining or extra data. The process involved adding a Dirichlet meta-model to a trained classifier over multiple intermediate layers which allowed the model to capture both epistemic and aleatoric uncertainty which led to promising results on various tasks such as out of domain detection, misclassification detection, and transfer learning. 

A related work, but distinct to the above works, in \cite{linusson2018classification} is an extension of conformal prediction \cite{vovk2005algorithmic} where a classification pipeline is enhanced to have a reject option. The process involves generating nonconformity scores (how strange an example is compared to the training data) on a calibration set, determining a threshold based on a desired error rate, and rejecting examples with low confidence. This technique has strong theoretical guarantees, bounding the probability of making incorrect predictions on accepted instances, allowing for reliable statistically-guaranteed control over uncertainty and risk in classification tasks. This method is also model and dataset agnostic, allowing it to be used for almost any problem with minimal restrictions. However, this method only typically considers the maximum predicted probability and does not consider the whole class distribution, or proximity the decision boundaries. 
...
The current work takes modest inspiration from the work in \cite{linusson2018classification} by obtaining uncertainty scores on the calibration set, and determining a threshold isolate regions with high uncertainty. Thus, the research question of this work is:

\begin{itemize}
    \item Can post-hoc uncertainty quantification measures be used to identify uncertain predictions?
\end{itemize}

\noindent The evaluation of these uncertain predictions takes the form of:

\begin{enumerate}
    \item Improving model performance
    \item Detecting adversarial examples
    \item Prediction misclassification
\end{enumerate}
...
\section{Experimental Setup - Maybe merge datasets and models?}
...
ImageNet is a large and famous dataset \cite{deng2009imagenet, ILSVRC15imagenet}. The dataset used in this work is the 2012 dataset with 1 million training, 50,000 validation, and 50,000 test images with 1,000 classes. The images are in colour and are of size 224x224. Three examples are shown in Figure \ref{fig:imagenet_examples}.
...
This specific version was used as pre-trained model weights for a ResNet-50 \cite{he2015deepresiduallearningimage} are available in PyTorch, and therefore no model training was required. Thus, the training data was not used, only the validation and test data. For the model specifications, see Section \ref{sec:imagenet_model}.
...
\subsubsection{ImageNet}
\label{sec:imagenet_model}

For the ImageNet dataset, two different models are used. The first is a pre-trained ResNet-50 described in \cite{he2015deepresiduallearningimage}, loaded with the default weights from TorchVision. For the full architecture, see \cite{he2015deepresiduallearningimage}. The second model uses the \texttt{yolov8n-cls} model \cite{Jocher_YOLO_by_Ultralytics_2023} with weights retrieved from \cite{Jocher_YOLO_by_Ultralytics_2023_weights}. Discussing the architectures here is beyond the scope of this work and we refer the reader to the provided citations.
...
Figure \ref{fig:results_alpha_boundary} shows the decision boundaries for the two-class 4x4 chessboard discussed above (for details see \ref{sec:chessboard_dataset}). The entropy threshold $\lambda$ was computed using Algorithm \ref{alg:compute_uncertainty_threshold} and the uncertain points identified using Algorithm \ref{alg:identifying_uncertain_points}.
...
\chapter{Case Study 3: Uncertainty Prediction}
\label{case:uncertainty_prediction}
...
add in the stuff on imagenet where the individual accuracy isnt as good as "at least one" and so they all find different things.

research question: can we quantify the model uncertainty post-hoc
...
Future work may explore if the same results may be obtained using the training data rather than a calibration set. This will bias the model, but for practical purposes it may be enough.
