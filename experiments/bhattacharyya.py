# taken from: https://github.com/EricPWilliamson/bhattacharyya-distance/blob/master/bhatta_dist.py
import numpy as np
from math import sqrt
from scipy.stats import gaussian_kde


def bhatta_dist(X1, X2, method="continuous", n_steps=200, cov_factor=0.1):
    """
    Calculate the Bhattacharyya distance between X1 and X2. X1 and X2 should be 1D numpy arrays representing the same feature in two separate classes.
    n_steps: the number of steps when method="continuous".
    ...
    if method == "noiseless":
        ...
        d1 = get_density(X1, cov_factor)
        d2 = get_density(X2, cov_factor)
        # Calc coeff:
        ...
        raise ValueError(
            "The value of the 'method' parameter does not match any known method"
        )
